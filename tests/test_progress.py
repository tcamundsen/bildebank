...
class ProgressLineTests(unittest.TestCase):
    ...
    def test_progress_meter_can_show_count_without_total(self) -> None:
        stream = TtyStringIO()
        progress = ProgressMeter("Import", stream=stream)

        progress.update_count(50, action="scannet", details="importert=3", force=True)
        progress.done()

        self.assertEqual(stream.getvalue(), "\rImport: scannet=50, importert=3\033[K\n")
