...
class MigrateCliTests(unittest.TestCase):
    ...
    def test_migrate_v2_removes_legacy_source_fk_and_kind_columns(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Ny schema_version: 19", stdout)
            conn = sqlite3.connect(target / DB_FILENAME)
            try:
                self.assertEqual(
    ...
    def test_migrate_check_reports_plan_without_changes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(list(target.glob(".bilder.sqlite3.backup-before-schema-19-*")))
            conn = sqlite3.connect(target / DB_FILENAME)
            try:
                self.assertFalse(
                    conn.execute(
    ...
    def test_migrate_backfills_file_sources_and_then_status_works(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(len(list(target.glob(".bilder.sqlite3.backup-before-schema-19-*"))), 1)
    ...
    def test_migrate_current_schema_repairs_missing_performance_index(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(code, 0, stderr)
            self.assertIn("Nåværende schema_version: 19", stdout)
    ...
    def test_migrate_rejects_invalid_current_schema_before_already_migrated(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(database_path.read_bytes(), before)
            self.assertFalse(
                list(target.glob(".bilder.sqlite3.backup-before-schema-19-*"))
            )
    ...
    def test_migrate_check_reports_internal_v11_repairs_without_changes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(code, 0, stderr)
            self.assertIn("reparere intern v19-struktur", stdout)
    ...
    def test_migrate_repairs_internal_v11_structure_and_preserves_tags_and_links(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(code, 0, stderr)
            self.assertIn("Reparerer intern v19-struktur.", stdout)
            self.assertEqual(len(list(target.glob(".bilder.sqlite3.backup-before-schema-19-*"))), 1)
            conn = sqlite3.connect(target / DB_FILENAME)
            try:
                collection_id = conn.execute(
    ...
    def test_migrate_v12_to_v19_adds_metadata_datetime_without_backfill(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "target"
            source = Path(tmp) / "source"
            source.mkdir()
    ...
    def test_migrate_v14_to_v19_adds_nullable_comment_without_changing_rows(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "target"
            source = Path(tmp) / "source"
            source.mkdir()
    ...
    def test_migrate_v15_to_v19_adds_pending_delete_identity_columns(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "target"
            self.assertEqual(run_cli(["create", str(target)]), 0)
            conn = sqlite3.connect(target / DB_FILENAME)
    ...
    def test_current_schema_rejects_v19_database_with_absolute_target_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            target = root / "target"
            source = root / "source"
    ...
    def test_current_schema_rejects_v4_database_with_legacy_tables_and_columns(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(code, 1)
            self.assertEqual(stdout, "")
            self.assertIn("schema_version=19", stderr)
            self.assertIn("bildebank migrate", stderr)
