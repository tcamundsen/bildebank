...
class ServerPeopleCliTests(unittest.TestCase):
    ...
    def test_run_server_people_page_links_confirmed_and_suggested_person_browser(
        ...
        self.assertIn('<table class="people-table">', body)
        self.assertIn('<th scope="col">Referansebilder</th>', body)
        self.assertIn('<th scope="col">Bekreftede &amp; forslag</th>', body)
        self.assertIn('<a class="person-link" href="/people/Kari/references">1 bilde</a>', body)
        self.assertIn('<a class="person-link" href="/person/Kari/no-faces">2 bilder</a>', body)
        self.assertIn('title="Endre" aria-label="Endre" data-open-person-rename', body)
        self.assertIn('data-person-name="Kari">✏️</button>', body)
        self.assertIn('title="Slett" aria-label="Slett" data-delete-person-name="Kari">🗑️</button>', body)
