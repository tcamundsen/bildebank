...
class MetadataTagsCliTests(unittest.TestCase):
    ...
    def test_tag_cli_and_server_browser(self) -> None:
        ...
        self.assertIn("<h2>Brukertagger</h2>", tags_body)
        self.assertIn('<div class="user-tag-table">', tags_body)
        self.assertIn('<div class="user-tag-header" aria-hidden="true">', tags_body)
        self.assertIn('<a class="person-link" href="/tag/Familie">1 bilde</a>', tags_body)
        self.assertIn("<h2>Systemtagger</h2>", tags_body)
        self.assertIn('<div class="system-tag-table">', tags_body)
